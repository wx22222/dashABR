...
COLOR_PALETTE = ["#EECA40", "#23BAC5", "#FD763F", "#E0A4DD", "#AA66EB"]
...
def plot_timeseries(times_list, series_list, names, out_path):
    ...
    ax.legend(loc="lower center",bbox_to_anchor=(0.5, 0.95), ncol=min(5, len(names)), frameon=False)
    ax = axs[1]
    for i in range(len(names)):
...
def main():
    base_dir = os.getcwd()
    p_lolp = os.path.join(base_dir, "output", "fcc", "dash_stats_lolp_fcc1.csv")
    p_l2a = os.path.join(base_dir, "output", "fcc", "dash_stats_l2a_fcc1.csv")
    p_custom = os.path.join(base_dir, "output", "fcc", "dash_stats_customrule_fcc1.csv")
    p_bola = os.path.join(base_dir, "output", "fcc", "dash_stats_bola_fcc1.csv")
    p_mpc = os.path.join(base_dir, "output", "fcc", "dash_stats_mpcrule_fcc1.csv")
    dt = 0.5
    args = sys.argv[1:]
    if len(args) >= 5:
        p_lolp = args[0]
    ...
    rows_mpc = load_rows(p_mpc)
    datasets = [("LoL+", rows_lolp), ("L2A-LL", rows_l2a), ("CustomRule", rows_custom), ("BOLA", rows_bola), ("MPCRule", rows_mpc)]
    ...
    out_summary = os.path.join(out_dir, "abr_time_norm_summary_five.svg")
    out_extra = os.path.join(out_dir, "abr_time_norm_extra_metrics_five.svg")
    out_ts = os.path.join(out_dir, "abr_time_norm_timeseries_five.svg")
    out_qdist = os.path.join(out_dir, "abr_time_norm_quality_dist_five.svg")
