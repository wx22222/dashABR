...
def kf_hca_predict_windows(X, omega=1e-5, mu=0.7, V=1.0):
    ...
    for i in range(X.shape[0]):
        ...
        if n == 0:
            preds.append(0.0)
            continue
        theta = -V / xh[0] if xh[0] != 0 else -1.0
        Z = 1.0
        P = 1.0
        for k in range(1, n):
            Tk = xh[k]
            U = 1.0
            M = -V / Tk if Tk != 0 else -1.0
            # Predict
            theta_pred = theta
            P_pred = P + omega
            # Update
            err = M - theta_pred
            S = P_pred * (U**2) + Z
            G = (P_pred * U) / S if S != 0 else 0
            theta = theta_pred + G * err
            P = P_pred - G * U * P_pred
            Z = mu * Z + (1 - mu) * (err**2)
        T_pred = -V / theta if theta != 0 else xh[-1]
        if T_pred < 0:
            T_pred = xh[-1]
...
def rls_predict_windows(X, filter_order=1, forgetting_factor=0.99, delta=1.0):
    """
...
def plot_summary_bars(m_lstm, m_ewma, m_arima, m_hca, m_rls, out_path):
    ...
    axs[0].set_title('Oboe MAE')
    axs[1].bar(names, rmses, color=colors)
    axs[1].set_title('Oboe RMSE')
    for ax in axs:
...
def plot_fit_timeseries(times, y, y_lstm, y_ewma, y_arima, y_hca, y_rls, out_path):
    ...
    ax.text(0.5, -0.2, "(b) Oboe", transform=ax.transAxes,
        ha="center", va="top")
    ax.set_xlabel('时间(s)')
...
def plot_step_response(ks, d_lstm, d_ewma, d_arima, d_hca, d_rls, out_path):
    ...
    def bars(ax, e, a, l, h, r, title):
        ...
        ax.legend()
    bars(axs[0], m_e, m_a, m_l, m_h, m_r, 'Oboe Step MAE')
    bars(axs[1], r_e, r_a, r_l, r_h, r_r, 'Oboe Step RMSE')
    bars(axs[2], o_e, o_a, o_l, o_h, o_r, 'Oboe Overshoot Rate')
    for ax in axs:
        ax.spines["right"].set_visible(False)
        ax.spines["top"].set_visible(False)
    fig.tight_layout()
...
def main():
    ...
    out_step = os.path.join(os.path.join(os.getcwd(), "assets", "images"), "predict_step_response_oboe.svg")
    try:
    ...
    try:
        ...
        pass
    out_summary = os.path.join(img_dir, "predict_compare_summary_oboe.svg")
    out_hist = os.path.join(img_dir, "predict_compare_error_hist_oboe.svg")
    out_scatter = os.path.join(img_dir, "predict_compare_scatter_oboe.svg")
            ...
            if Xi is not None and yi is not None and len(yi) > 0:
                ...
                if args.example_t0 is not None and args.example_t1 is not None:
                    try:
                        ...
                        pass
                out_fit = os.path.join(img_dir, "predict_fit_example_oboe.svg")
